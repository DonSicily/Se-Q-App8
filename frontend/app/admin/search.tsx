...
import { View, Text, TouchableOpacity, StyleSheet, ScrollView, TextInput, FlatList, ActivityIndicator, Alert, Platform, BackHandler} from 'react-native';
...
<<<<<<< HEAD
            <View style={styles.resultTypeRow}>
              ...
              {item.role && (
                <View style={[styles.roleBadge, { backgroundColor: `${iconInfo.color}30` }]}>
                  <Text style={[styles.roleBadgeText, { color: iconInfo.color }]}>
...
<<<<<<< HEAD
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterRow}>
          {(['all', 'civil', 'security', 'admin'] as UserRole[]).map((role) => (
            <TouchableOpacity
              key={role}
              style={[styles.filterChip, userRole === role && styles.filterChipActive]}
              onPress={() => setUserRole(role)}
            >
              <Text style={[styles.filterChipText, userRole === role && styles.filterChipTextActive]}>
