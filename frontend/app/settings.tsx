...
export default function Settings() {
  ...
  const [msgSoundEnabled, setMsgSoundEnabled] = useState(true);
  ...
  const initializeSettings = async () => {
    ...
    const stored = await AsyncStorage.getItem('msg_sound_enabled');
    setMsgSoundEnabled(stored !== 'false'); // default true
    await loadProfile();
              ...
              <View style={styles.notifRow}>
                <View style={styles.notifLeft}>
                  ...
                  <View style={styles.notifInfo}>
                    <Text style={styles.notifTitle}>Message Sound Alert</Text>
                    <Text style={styles.notifDesc}>
                      Play a sound when a new message arrives in your Message Centre
                    </Text>
                  </View>
                </View>
                <Switch
                  value={msgSoundEnabled}
                  onValueChange={async (val) => {
                    setMsgSoundEnabled(val);
                    await AsyncStorage.setItem('msg_sound_enabled', val ? 'true' : 'false');
                  }}
                  trackColor={{ false: '#334155', true: '#3B82F6' }}
                  thumbColor={msgSoundEnabled ? '#fff' : '#94A3B8'}
                />
              </View>
              <Text style={styles.notifHint}>
                {msgSoundEnabled
                  ? '🔔 Sound alert is ON — you will hear a chime when a new message arrives'
                  : '🔕 Sound alert is OFF — new messages will arrive silently'}
              </Text>
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  notifCard: { backgroundColor: '#1E293B', borderRadius: 16, padding: 16 },
  notifRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 },
  ...
  notifInfo: { flex: 1 },
  notifTitle: { fontSize: 15, fontWeight: '600', color: '#fff', marginBottom: 3 },
  notifDesc: { fontSize: 12, color: '#64748B', lineHeight: 16 },
  notifHint: { fontSize: 12, color: '#475569', fontStyle: 'italic', paddingTop: 8, borderTopWidth: 1, borderTopColor: '#334155' },
  // Camera modal
